# Runtime Reliability and Architecture Hardening Implementation Plan
...
Design authority:
[Runtime Reliability and Architecture Hardening](runtime-reliability-and-architecture-hardening.md)

Supersedes the execution plan in
[Runtime and HTTP Contract Hardening Implementation Plan](runtime-http-contract-hardening-implementation-plan.md)
without modifying that historical plan.
...
24. Configure Uvicorn with disabled access and proxy-header trust, one explicit
    process concurrency bound, and a drain budget that is a strict partition of
    the runtime shutdown budget.
25. Make body admission exact over method and path and replace queued
    semaphores with immediate no-queue leases. Add independent no-queue token
    buckets for public OAuth start and callback routes.
26. Move generic unexpected-error translation inside correlation ownership so
    every redacted `500` retains the correlation header.
27. Put credential acquisition, refresh admission, and provider exchange inside
    one absolute GitHub deadline. Bound unit-of-work finalization with a separate
    cleanup deadline and preserve the primary error without waiting forever.
28. Admit webhook media and encoding before trust work. Execute signature,
    hash, strict JSON, event-family binding, and normalization through one
    cancellable no-queue worker process. Keep only the raw request-shaped port
    at the HTTP consumer boundary, implement it in runtime composition, and
    delete the policy-free application pass-through.
29. Bind durable webhook replay to signed body identity as well as provider
    delivery metadata. A repeated signed body cannot create another effect
    under changed unsigned headers.
30. Freeze the audit-ledger head, verify bounded pages in order, and stream CLI
    output only after a complete first verification pass. Never materialize the
    production ledger as one tuple.
31. Classify only typed reconciliation availability failures as retry-safe and
    derive every nonterminal deferral's equal jitter deterministically from
    exact subject and attempt identity. Measure mutation duration with a
    monotonic clock.
32. Move rollback command and admission ownership to `config_epochs`, implement
    its consumer-owned coverage port in `planning_core`, use capability-owned
    governance and workflow ports, route override construction through typed
    domain admission, and reject application imports of integrations or runtime.
33. Harden production launch examples and publish an owner-run capacity
    qualification procedure. Keep capacity and provider-wide rate assurance as
    explicit external non-claims until exact evidence exists.
34. Restore the published bootstrap migration byte-for-byte. Add exactly one
    forward migration that preserves every generation-two descriptor, adds a
    separately attested `webhook-body-identity/v1` capability, and proves fresh,
    generation-two-to-head, downgrade, replay, and duplicate-backfill behavior.
35. Replace combined delivery-id/body lookup with a total ordered classifier:
    delivery id first, body identity only when that id is absent. Repair or
    verify the original pair-owned audit event before returning a body duplicate,
    and add cross-collision plus orphan-body falsifiers.
36. Reserve positive time for pinned Uvicorn orchestration outside its graceful
    timeout, then partition the remainder between server drain and application
    cleanup. Bind reconciliation drain to half the application budget, not half
    the total process budget.
37. Replace the application-layer infrastructure denylist with a closed
    first-party capability allowlist. Retain explicit technical prohibitions and
    prove that current and future CLI/runtime/provider packages remain rejected.
38. Stage the second audit-replay pass in an owner-private temporary file and
    publish fixed-size chunks only after one complete JSON document closes.
    Specify exit-zero plus complete parsing as the affirmative receipt and retain
    output-sink partial writes as a non-claim.
39. Reject non-finite token-bucket clock samples without mutation and treat
    backward samples as zero elapsed time without moving the watermark. Add
    parent-side webhook serialization latency and event-loop heartbeat delay to
    production capacity qualification; do not replace IPC without a failed
    owner budget.
40. Refresh the code-owned database compatibility profile digest in the same
    change as its bundled and documentation inventories. Prove byte equality,
    exact digest admission, and generation-two-to-generation-three declaration
    continuity before publishing the migration. Reject downgrade when either a
    webhook claim or its pair-owned audit evidence has been retained.
...
| Input-port ownership                    | capability ports, application orchestration, or exact HTTP transport boundary  | type checking, complete HTTP Protocol inventory, import-boundary negative probes, structural fake                                                                                                                |
...
| Database compatibility profile identity | bundled profile, documentation projection, and pinned digest                   | byte equality, exact SHA-256 admission, migration declaration continuity                                                                                                                                         |
...
| Server and public-route admission       | Uvicorn entry point and HTTP middleware                                        | explicit proxy/log/concurrency settings, method-path body leases, OAuth token-bucket boundaries                                                                                                                  |
| Webhook CPU and replay identity         | HTTP, `identity_admission`, `github_ingestion`, persistence                    | media/encoding rejection, worker cancellation, event-family binding, body-identity duplicate/conflict witnesses                                                                                                  |
| Audit replay resource bound             | `audit_replay`, replay CLI, PostgreSQL adapter                                 | frozen-head pages, cross-page chain falsifiers, bounded writer calls                                                                                                                                             |
| Audit replay publication                | replay CLI and temporary-file owner                                            | second-pass failure before publication, bounded copy chunks, exit/parse receipt                                                                                                                                  |
| Provider and cleanup deadlines          | GitHub transport and persistence unit of work                                  | blocked credential/refresh/finalizer deadline witnesses                                                                                                                                                          |
| Reconciliation retry                    | reconciliation round service                                                   | unknown-defect propagation and deterministic equal-jitter witnesses                                                                                                                                              |
| Capability ownership                    | context modules and import policy                                              | canonical port identity, removed pass-through, provider/runtime negative probes                                                                                                                                  |
| Migration lineage                       | Alembic generation three and capability attestors                              | published SHA pins, fresh and generation-two upgrades, duplicate-body rejection                                                                                                                                  |
| Webhook claim totality                  | delivery repository and pair-owned audit adapter                               | id-first cross-collision, orphan audit repair, concurrent body identity                                                                                                                                          |
| Clock anomaly admission                 | public request limiter                                                         | non-finite initialization/sample, backward watermark, later recovery                                                                                                                                             |
| Production operations                   | deployment and capacity how-to                                                 | privilege flags, bearer rotation, exact-epoch capacity procedure                                                                                                                                                 |
...
Accept iff
  ...
  and application first-party imports are closed to its admitted capability set
  and one domain invariant or capability contract has exactly one semantic owner
  and HTTP declares only request-shaped transport protocols
  and body admission has no local waiter queue and is exact over method and path
  and OAuth start and callback have finite independent local admission budgets
  and Uvicorn logs no query-bearing access line and trusts no forwarded header
  and one webhook body hash can publish at most one durable effect
  and webhook event metadata cannot reinterpret the signed payload family
  and webhook CPU work enters no ambient worker queue
  and parent-side process serialization has an owner-qualified latency budget
  and audit replay memory is bounded by its page and output chunk, not ledger size
  and second-pass failure publishes no affirmative JSON prefix
  and GitHub deadlines include credential acquisition and refresh admission
  and unit-of-work cleanup has a finite owner deadline
  and only typed availability failures are classified retry-safe
  and every nonterminal deferral uses deterministic equal jitter
  and elapsed mutation evidence uses a monotonic clock
  and all cleanup callers share one owner task and one total deadline
  and framework reserve plus server drain plus resource cleanup equal the total budget
  and reconciliation drain is half the application-cleanup budget
  ...
  and production examples remove ambient capabilities, privilege escalation, and writable root state
  and bundled and documented database compatibility profiles are byte-identical
  and the pinned profile digest admits those exact bytes
  and every published migration remains byte-identical to its baseline artifact
  and the head migration preserves the exact predecessor declaration
  and head downgrade cannot weaken retained webhook-body identity
...
30. Can an admitted request wait in any unbounded in-process queue before its
    owner deadline begins?
31. Can changed unsigned webhook metadata create a second effect or reinterpret
    one authenticated body?
32. Can replay of a ledger larger than process memory still verify and stream
    with a fixed page bound?
33. Does any retry catch an exception whose type fails to prove temporary
    dependency unavailability?
34. Do server drain, application cleanup, and container grace periods form one
    satisfiable strict inequality rather than additive independent limits?
35. Can one incoming `(delivery_id, body_hash)` match two different stored rows,
    and if so does delivery-id conflict still classify deterministically?
36. Can a database at either fresh base or published generation two reach the
    same declared head without rewriting a published migration or descriptor?
37. Can a failure on replay pass two expose bytes that a consumer may mistake
    for an affirmative complete JSON receipt?
38. Can a non-finite or backward clock sample mint OAuth refill capacity?
39. Can a newly added first-party technical package enter `app` without an
    explicit owner-reviewed allowlist change?
...
The published bootstrap and governance-baseline expansion remain immutable.
Webhook body identity is generation-three forward history. A source rollback
must therefore retain that migration and its capability definition until the
database is explicitly downgraded through the admitted generation-two contract;
it must never rewrite, delete, or squash either published predecessor.
